...
export default function DriverHistoryScreen() {
  const { data, isLoading, isError } = useDriverHistory();
  ...
  if (isError) {
    return (
      <ScreenContainer bg={colors.backgroundMuted}>
        <AppHeader title="My Trips" subtitle="History & efficiency" onBack={() => router.back()} />
        <EmptyState title="Could not load history" description="Check your connection and try again" />
      </ScreenContainer>
    );
  }
  ...
  return (
    <ScreenContainer bg={colors.backgroundMuted}>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View>
            {data?.summary && <EfficiencySummary summary={data.summary} />}
            <Text style={styles.sectionTitle}>PAST TRIPS · {trips.length}</Text>
          </View>
        }
        ListEmptyComponent={
          <EmptyState title="No past trips yet" description="Completed trips will show up here" />
        }
        renderItem={({ item }: { item: HistoryTrip }) => {
